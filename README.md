...
- ⚙️ Per evitare che `react-pageflip` ricrei l’HTML a ogni render e faccia perdere il focus all’editor, usiamo il wrapper `FocusSafeHTMLFlipBook` (`src/components/flipbook/FocusSafeHTMLFlipBook.tsx`) che mantiene `renderOnlyPageLengthChange` attivo e richiama `loadFromHTML` / `updateFromHtml` solo quando cambia il numero/ordine delle pagine. Cambia lo spread solo se varia il numero di pagine.
